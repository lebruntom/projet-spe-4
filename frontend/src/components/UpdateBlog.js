...
import { updateBlog } from "../axios/blog";
import { AuthContext } from "../store/AuthContext";
import { showToastMessage } from "../utils/common";

const UpdateBlog = ({ setNeedRefresh, blog }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      //Call api pour mettre à jour les blogs
      updateBlog({ email, subject, title, status }, blog.id)
        .then((res) => {
          setNeedRefresh(true);
          showToastMessage("Article modifié", "success");
          formik.resetForm();
        })
        .catch((err) => {
